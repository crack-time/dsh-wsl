...
export function globFindCmd(path: string, pattern: string): string {
...
export function writeFileCmd(path: string, contentB64: string): string {
 ...
 */
export function editFileCmd(path: string, oldB64: string, newB64: string, replaceAll: boolean): string {
  const file = shellQuote(path || '/')
...
//
// WHAT the shell guard covers, deliberately: only the SIMPLE, easy-to-trigger
// write/copy/delete primitives (rm, rmdir, unlink, mv, mkdir, touch, truncate,
// ln, with cp checking only its destination). These are the commands a model
// reaches for reflexively, so they get intercepted. The COMPLEX writers — `>`
// /`>>` redirections, `sed -i`, `python open(...,'w')`, `dd of=`, `tee`, archive
// extraction — are intentionally NOT intercepted: a model that writes them out is
// acting deliberately, so that deliberation itself is the review the sandbox wants.
// (They would also be hard to intercept from a bash prefix — shell-syntax
// redirection and embedded file-open calls run outside the shadowed-function net.)
//
