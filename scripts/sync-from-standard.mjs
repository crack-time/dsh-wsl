...
function findStandardDir(explicit) {
  ...
  const candidates = [
    // npm global install of dsh
    process.env.APPDATA || '',
    process.env.HOME || '',
  ].flatMap((base) => base ? [
    // npx/npm global layout
  ] : []);
  const globalRoots = [
    ...[process.env.APPDATA ? join(process.env.APPDATA, 'npm', 'node_modules', '@deepseek-ai', 'dsh', 'node_modules', '@deepseek-ai', 'dsh-agent-presets', 'presets', 'standard') : []],
    ...[process.env.NPM_CONFIG_PREFIX ? join(process.env.NPM_CONFIG_PREFIX, 'node_modules', '@deepseek-ai', 'dsh', 'node_modules', '@deepseek-ai', 'dsh-agent-presets', 'presets', 'standard') : []],
...
const TOOL_WSL_BLOCK = `- id: tool-wsl
...
function transform(standardAgentYml) {
...
const regenerated = transform(source)
