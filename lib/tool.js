...
import { buildScript, distroOf, editFileCmd, globFindCmd, grepCmd, readWindowCmd, toBase64, toWslPath, wslArgv, writeFileCmd, resolveWorkdir, shellQuote, DEFAULT_DISTRO, DEFAULT_TIMEOUT_MS, MAX_TIMEOUT_MS, } from './wsl-util.js';
...
export function apply(_ctx, config = {}) {
    ...
    async function execWslText(exec, command, workdirArg, envArg) {
        const sessionCwd = exec.agent?.session?.header?.cwd;
