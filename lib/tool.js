...
import { buildScript, distroOf, editFileCmd, globFindCmd, grepCmd, readWindowCmd, toBase64, toWslPath, wslArgv, writeFileCmd, resolveWorkdir, shellQuote, workspaceWriteGuard, assertWslPathInside, DEFAULT_DISTRO, DEFAULT_TIMEOUT_MS, MAX_TIMEOUT_MS, } from './wsl-util.js';
 ...
 */
async function deployDaemonPayload(ctx, distro, signal) {
    try {
        ...
        for (const rel of DAEMON_PAYLOAD) {
            ...
            if (!existsSync(src))
                continue; // daemon/ not shipped in this install — skip
...
function daemonOp(host, port, token, payload) {
    return new Promise((resolve, reject) => {
        const socket = connect({ host, port });
 ...
 */
async function startDaemonBackground(host, port, token, req) {
    ...
    if (typeof r.jobId !== 'string' || !r.jobId)
        throw new Error(`wsl daemon bg start failed: ${String(r.error ?? '')}`);
    return r.jobId;
}
function makeDaemonBackgroundHandle(host, port, token, jobId) {
...
async function runDaemonForegroundBackend(d) {
        ...
        if (daemonAutoStart && now - lastDaemonStart >= DAEMON_AUTO_START_THROTTLE_MS) {
            ...
            if (!daemonLaunching) {
                daemonLaunching = (async () => {
                    try {
                        await deployDaemonPayload(ctx, distro, exec.signal);
                    }
                    catch { }
                    try {
                        await launchDaemonViaWsl(ctx, distro, exec.signal, daemonToken);
                    }
                    catch { }
            ...
            try {
                ...
                return { note: `[wsl daemon unreachable after auto-start at ${daemonHost}:${daemonPort} — fell back to one-shot bridge; state will NOT persist this call] (${e2.message})` };
            }
        }
        return { note: `[wsl daemon unreachable at ${daemonHost}:${daemonPort} — fell back to one-shot bridge; state will NOT persist this call] (${e.message})` };
    }
}
...
function daemonLaunchBash(token) {
...
async function launchDaemonViaWsl(ctx, distro, signal, token) {
    try {
        const handle = ctx.subprocess.spawn({
...
function resolveSandboxMode(ctx, exec) {
    try {
...
export function apply(_ctx, config = {}) {
    ...
    async function execWslText(exec, command, workdirArg, envArg, mutation) {
        const sessionCwd = exec.agent?.session?.header?.cwd;
        ...
        let dshEnv = ctx.shellEnv.collect(exec);
        if (envArg) {
        ...
        if (daemonEnabled) {
            ...
            if ('value' in outcome)
                return renderResult(outcome.value);
